...
int flagmouse = 1; //确定是读文件 还是写文件 1代表读 0代表写
int flagkey = 1;//默认是读文件
...
string g_mouse_dev[100] = {};
string g_keyboard_dev[100] = {};

std::fstream accfilemouse;
...
void mouseIdInit() {
    if(flagmouse == 1) {
        //读取文件到g_mouse_dev
        string s = "";
        accfilemouse.open("mouseid.txt");
        if(!accfilemouse.is_open()) {
            qDebug() << "open failed" << endl;
        }
        int i = 0;
        while(getline(accfilemouse, s)) {
            const char *p = s.data();
            g_mouse_dev[i] = p;
            if(g_mouse_dev[i] == p)
                qDebug() << QString::fromStdString(g_mouse_dev[i]) ;
            i ++;
        }
        if(accfilemouse.is_open()) {
            accfilemouse.close();
        ...
        accfilemouse.open("mouseid.txt", ios::out);
    }
}

std::fstream accfilekey;
void keyIdInit() {
    if(flagkey == 1) {
        string s = "";
        accfilekey.open("keyid.txt");
        if(!accfilekey.is_open()) {
            qDebug() << "open failed" << endl;
        }
        int i = 0;
        while(getline(accfilekey, s)) {
            const char *p = s.data();
            g_keyboard_dev[i] = p;
            if(g_keyboard_dev[i] == p)
                qDebug() << QString::fromStdString(g_keyboard_dev[i]) ;
            i ++;
        }
        if(accfilekey.is_open()) {
            accfilekey.close();
        ...
        accfilekey.open("keyid.txt", ios::out);
    }
}
